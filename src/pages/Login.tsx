...
export default function Login() {
    ...
    try {
      if (isRegister) {
        if (parseInt(captchaAnswer) !== captchaNum1 + captchaNum2) {
          setError("La respuesta de seguridad (CAPTCHA) es incorrecta.");
          setLoading(false);
          resetCaptcha();
          return;
        }
      ...
      let errorMessage =
        "Fallo en la autenticación. Por favor, inténtelo de nuevo.";
  ...
  const handleResetPassword = async () => {
    ...
    try {
      ...
      setError(
        "Se ha enviado un correo para restablecer tu contraseña. (Revisa la carpeta de spam)",
      );
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <div className="min-h-screen bg-[#050505] flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative">
      <div className="absolute top-6 left-6">
        <button
          onClick={() => navigate("/")}
          className="text-sm font-bold text-gray-500 hover:text-white flex items-center gap-2 transition-colors"
        >
          ← Volver al inicio
        </button>
      </div>
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <h2 className="mt-6 text-center text-3xl font-bold tracking-tight text-white">
          Servidor Maestro{" "}
          <span className="font-light text-gray-400">ONYX</span>
        </h2>
            ...
            {isRegister && (
              <div className="bg-black/30 p-4 rounded-lg border border-white/5 mt-2">
                ...
                <div className="flex items-center gap-4">
                  <div className="bg-black/80 px-4 py-2 rounded-lg border border-white/10 text-white font-mono text-lg font-bold tracking-widest select-none">
